      ...
      (res) => {
        ...
        if (!res) {
          ...
          resolve({
            ...
            color: "yellow",
            greenLevel: null,
            reasons: ["No response from extension"],
            tip: "ERROR",
            alternatives: []
          });
          return;
        ...
        try {
          ...
          parsed.ecoScore = parsed.ecoScore ?? 5;
          parsed.color = parsed.color ?? "yellow";
          parsed.greenLevel = parsed.greenLevel ?? 50;
          parsed.reasons = parsed.reasons ?? ["Eco analysis complete"];
          parsed.tip = parsed.tip ?? "Check product certifications";
          parsed.alternatives = parsed.alternatives ?? [];
          
          console.log("✅ Parsed eco score:", parsed);
          resolve(parsed);
        } catch (error) {
          console.error("❌ Error parsing response:", error, res);
          resolve({
            ecoScore: null,
            color: "yellow",
            greenLevel: null,
...
async function performSearch() {
  ...
  resultDiv.innerHTML = `<div style="text-align:center;"><div style="font-size:18px;color:#43a047;">🔎 Scanning...</div><div style="margin-top:4px;font-size:12px;color:#666;">Analyzing product's eco-impact...</div></div>`;
  const score = await generateEcoScore(input, "shopping");
  
  if (!score || score.tip === "ERROR" || score.ecoScore === null) {
    resultDiv.innerHTML = "<p style='color:red;'>❌ Could not analyze product. Try again.</p>";
    return;
  }
  
  const ecoColor = getEcoColor(score.ecoScore);
  const barWidth = Math.min(100, Math.max(0, Math.round(score.greenLevel || score.ecoScore * 10)));
  ...
  (score.alternatives || []).forEach((alt) => {
    altsHTML += `<a href='https://www.amazon.in/s?k=${encodeURIComponent(alt.name)}' target='_blank' style='background:white;padding:6px;margin:2px 0;border-radius:4px;text-align:center;font-size:10px;display:inline-block;color:#2e7d32;text-decoration:none;font-weight:600;box-shadow:0 1px 3px #0001;'>${alt.name} (${Math.round(alt.greenScore || 75)}%)</a>`;
  });
  altsHTML += "</div>";
  resultDiv.innerHTML = `
...
async function performCompare() {
  ...
  if (!s1 || !s2 || s1.ecoScore === null || s2.ecoScore === null) {
    resultDiv.innerHTML = "<p style='color:red;'>❌ Could not compare products</p>";
    return;
  }
  
  const color1 = getEcoColor(s1.ecoScore);
  const color2 = getEcoColor(s2.ecoScore);
  const w1 = Math.min(100, Math.max(0, Math.round(s1.greenLevel || s1.ecoScore * 10)));
  const w2 = Math.min(100, Math.max(0, Math.round(s2.greenLevel || s2.ecoScore * 10)));
