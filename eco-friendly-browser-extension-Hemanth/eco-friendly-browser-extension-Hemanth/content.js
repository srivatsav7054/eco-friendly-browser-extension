...
function createEcoPanel() {
  ...
  const sustainabilityPromise = new Promise((resolve) => {
  ...
  const ecoScorePromise = new Promise((resolve) => {
    chrome.runtime.sendMessage(
      { action: "generateEcoScore", product: productTitle, category: "shopping" },
      (response) => {
  ...
  Promise.all([sustainabilityPromise, ecoScorePromise]).then(([sustainData, ecoData]) => {
    // Merge data: use Ollama data for scores/alternatives, but keep sustainability for product name
    const mergedData = {
      productName: sustainData.productName || productTitle,
      carbonFootprint: ecoData.carbonFootprint || sustainData.carbonFootprint || "Unknown",
      greenLevel: ecoData.greenLevel ?? sustainData.greenLevel ?? 50,
      alternatives: ecoData.alternatives?.length ? ecoData.alternatives : sustainData.alternatives || [],
      ecoReason: ecoData.reasons?.[0] || "Product analysis complete",
      tip: ecoData.tip || "Check for eco-certifications",
    };

    if (!mergedData.productName || mergedData.productName === "Unknown") {
      heading.textContent = "⚠️ Error loading product.";
      return;
    }

    fillEcoPanel(panel, mergedData);
  });
}
...
function fillEcoPanel(panel, data) {
  ...
  content.innerHTML = `
    ...
    ${data.ecoReason ? `<div style="background:#e8f5e9;border-radius:6px;padding:10px;margin:12px 0;font-size:12px;color:#2e7d32;"><strong>🤖 Analysis:</strong> ${escapeHtml(data.ecoReason)}</div>` : ''}

    ${data.tip ? `<div style="background:#fff3cd;border-radius:6px;padding:10px;margin:12px 0;font-size:12px;color:#856404;"><strong>💡 Tip:</strong> ${escapeHtml(data.tip)}</div>` : ''}
